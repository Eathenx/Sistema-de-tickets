...
import {
  ...
  TrendingUp, TrendingDown, Inbox, HelpCircle
} from "lucide-react";
...
// =============================================================
const KPI_CONFIG = {
  pending:  { label: "Tickets pendientes", icon: Clock,         bg: "bg-amber-50",  iconColor: "text-amber-600",  trend: "+2 desde ayer",     trendUp: false },
  resolved: { label: "Resueltos hoy",      icon: CheckCircle2,  bg: "bg-emerald-50",iconColor: "text-emerald-600", trend: "+18% vs. ayer",      trendUp: true  },
  urgent:   { label: "Urgentes abiertos",  icon: AlertCircle,   bg: "bg-red-50",    iconColor: "text-red-500",    trend: "−1 desde ayer",      trendUp: true  },
  avgTime:  { label: "Tiempo prom. resolución", icon: Gauge,    bg: "bg-violet-50", iconColor: "text-violet-600", trend: "−30min vs. sem. ant.", trendUp: true  },
};
...
function KpiCard({ type, value }) {
  ...
  const trendColor = cfg.trendUp ? "text-emerald-600" : "text-red-500";
  return (
...
const PRIO_STYLES = {
  urgent: { badge: "bg-red-50 text-red-800",    dot: "bg-red-500" },
  high:   { badge: "bg-orange-50 text-orange-800", dot: "bg-orange-500" },
  medium: { badge: "bg-yellow-50 text-yellow-800", dot: "bg-yellow-400" },
  low:    { badge: "bg-green-50 text-green-800",  dot: "bg-green-500" },
};
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2 flex-wrap">
        <span className="text-sm font-medium text-gray-900 flex-1">Todos los tickets</span>
          ...
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              {["ID","Título / Categoría","Solicitante","Agente","Estado","Prioridad",""].map((h, i) => (
                <th key={i} className="px-4 py-2.5 text-left text-[11px] font-medium text-gray-400 uppercase tracking-wide whitespace-nowrap">
                  {h}
                </th>
              ))}
            </tr>
            ...
            {tickets.length === 0 ? (
              <tr>
                <td colSpan={7} className="text-center py-12 text-gray-400 text-sm">
                  <Inbox size={32} className="mx-auto mb-2 text-gray-200" />
              ...
              tickets.map((t) => (
                <tr
                  ...
                  className="border-b border-gray-50 hover:bg-gray-50 cursor-pointer transition-colors last:border-none"
                >
                  ...
                  <td className="px-4 py-3 text-gray-600 text-xs whitespace-nowrap">{t.requester}</td>
                  <td className="px-4 py-3 text-gray-600 text-xs whitespace-nowrap">{t.agent}</td>
...
function TicketDetail({ ticket, agents, onClose, onSave }) {
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl w-[440px] max-h-[90vh] overflow-y-auto border border-gray-100 shadow-xl">
        {/* Header */}
        <div className="px-5 py-4 border-b border-gray-100 flex items-start gap-3">
          <div className="flex-1">
            <p className="font-mono text-[11px] text-gray-400 mb-1">{ticket.id}</p>
            <p className="text-sm font-medium text-gray-900">{ticket.title}</p>
            <p className="text-xs text-gray-400 mt-0.5">{ticket.category} · {ticket.created}</p>
          </div>
          <button onClick={onClose} className="w-7 h-7 rounded-lg border border-gray-200 flex items-center justify-center text-gray-400 hover:bg-gray-50">
            <X size={14} />
          </button>
        </div>
        ...
        <div className="px-5 py-4 flex flex-col gap-4">
          <div>
            <p className="text-[10px] font-medium text-gray-400 uppercase tracking-wide mb-1.5">Descripción</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><p className="text-[10px] font-medium text-gray-400 uppercase tracking-wide mb-1">Estado</p><StatusBadge status={ticket.status} /></div>
            <div><p className="text-[10px] font-medium text-gray-400 uppercase tracking-wide mb-1">Prioridad</p><PriorityBadge priority={ticket.priority} /></div>
            <div><p className="text-[10px] font-medium text-gray-400 uppercase tracking-wide mb-1">Solicitante</p><p className="text-sm text-gray-900">{ticket.requester}</p></div>
            <div><p className="text-[10px] font-medium text-gray-400 uppercase tracking-wide mb-1">Creado</p><p className="text-sm text-gray-900">{ticket.created}</p></div>
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400"
            >
          ...
          <div>
            <p className="text-[10px] font-medium text-gray-400 uppercase tracking-wide mb-3">Actividad reciente</p>
            {[
        ...
        <div className="px-5 py-3 border-t border-gray-100 flex justify-end gap-2">
          <button onClick={onClose} className="px-3 py-1.5 rounded-lg border border-gray-200 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
            Cancelar
          </button>
          <button
            onClick={handleSave}
            className="px-4 py-1.5 rounded-lg bg-violet-700 text-white text-sm font-medium hover:bg-violet-800 transition-colors flex items-center gap-1.5"
          >
...
function DashboardView({ stats, tickets, onNavigate, onOpenTicket }) {
  const STATUS_INFO = [
    ...
    { key: "closed",   label: "Cerrados",   color: "bg-gray-300" },
  ];
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <div>
          <p className="text-base font-medium text-gray-900">Bienvenido de nuevo, Admin</p>
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 bg-violet-700 text-white text-sm font-medium rounded-lg hover:bg-violet-800 transition-colors"
        >
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100 flex items-center">
            <span className="text-sm font-medium text-gray-900 flex-1">Tickets recientes</span>
            <button onClick={() => onNavigate("tickets")} className="text-xs text-violet-600 hover:underline">Ver todos →</button>
          </div>
          {tickets.slice(0, 4).map((t) => (
            <div
              key={t.id}
              onClick={() => onOpenTicket(t.id)}
              className="px-4 py-2.5 flex items-center gap-3 border-b border-gray-50 last:border-none hover:bg-gray-50 cursor-pointer transition-colors"
            >
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-900 truncate">{t.title}</p>
                <p className="text-[11px] text-gray-400">{t.id} · {t.requester}</p>
              </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100">
            <span className="text-sm font-medium text-gray-900">Distribución por estado</span>
          </div>
                ...
                <div key={key}>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-xs font-medium text-gray-900">
                      {row?.count ?? 0} <span className="text-gray-400 font-normal">({pct}%)</span>
                    </span>
                  </div>
  ...
  return (
    <aside className="w-56 bg-white border-r border-gray-100 flex flex-col flex-shrink-0">
      ...
      <div className="h-14 px-4 flex items-center gap-2.5 border-b border-gray-100">
        ...
        <span className="text-sm font-medium text-gray-900 tracking-tight">HelpDesk Pro</span>
      </div>

      {/* Nav */}
      <nav className="flex-1 p-2.5 flex flex-col gap-0.5">
        <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest px-2 py-2">Principal</p>
        ...
        <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest px-2 py-2 mt-1">Gestión</p>
      ...
      <div className="p-3 border-t border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-gray-400 truncate">admin@helpdesk.io</p>
          </div>
          <button className="text-gray-300 hover:text-gray-500 transition-colors" title="Cerrar sesión">
            <LogOut size={14} />
          </button>
        </div>
      </div>
...
function Topbar({ activeView, search, onSearch }) {
  const [hasNotif, setHasNotif] = useState(true);
  return (
    <header className="h-14 bg-white border-b border-gray-100 flex items-center px-5 gap-3 flex-shrink-0">
      <span className="text-sm font-medium text-gray-900 flex-1">{VIEW_TITLES[activeView] ?? activeView}</span>
      {/* Search — filtra la tabla de tickets */}
      <div className="flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-lg px-3 h-8">
        <Search size={13} className="text-gray-400" />
        <input
          type="text" placeholder="Buscar tickets..."
          value={search} onChange={(e) => onSearch(e.target.value)}
          className="bg-transparent text-sm text-gray-900 outline-none w-44 placeholder:text-gray-400"
        />
        {search && (
...
// =============================================================
function AdminLayout({ children, activeView, onNavigate, search, onSearch, pendingCount }) {
  return (
    <div className="flex h-screen bg-gray-50 font-sans">
      <Sidebar activeView={activeView} onNavigate={onNavigate} pendingCount={pendingCount} />
      <div className="flex-1 flex flex-col min-w-0">
        <Topbar activeView={activeView} search={search} onSearch={onSearch} />
